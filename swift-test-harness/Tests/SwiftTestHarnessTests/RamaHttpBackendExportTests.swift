...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `RamaHttpBackend.swiftmodule/`
...
//      `import RamaHttpBackend` above would fail at compile time.
//
//   2. The static archive `libRamaHttpBackend.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "RamaHttpBackend" }` and
//      `flattenPackage = "io.github.kotlinmania.ramahttpbackend"` configuration in
...
//      via the `RamaHttpBackendLibrary` product.
//
...
final class RamaHttpBackendExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "RamaHttpBackend swift module imported cleanly")
    }
}
