# Claude Code Project Instructions — rama-http-backend-kotlin

## Project Overview

This is **rama-http-backend-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-http-backend`](https://crates.io/crates/rama-http-backend).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-http-backend/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-http-backend>
